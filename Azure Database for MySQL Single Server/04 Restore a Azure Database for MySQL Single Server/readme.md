...
**Exercise list**
- [Restore an Azure Database for MySQL Single Server to a point in time](#restore-an-azure-database-for-mysql-single-server-to-a-point-in-time)
  - [Exercise 1: Restore a server to a point in time](#exercise-1-restore-a-server-to-a-point-in-time)
  - [Exercise 2: Restore a Deleted Azure Database for MySQL Single Server](#exercise-2-restore-a-deleted-azure-database-for-mysql-single-server)
...
1. Access the restored server
   ...
   Go to the MySQL Server you create in the previous step using your preferred method.
    
   ![Image0066](Media/image0066.png)

   >Now you can administer and access this server as you do with any other Azure Database for MySQL. You can change the administrator password, change the backup retention policies, change the pricing tier and do any administrative task you want as this is server independent of the one you restored from.
...
1. Recover the deleted server 
   ...
   Go to Create Server [REST API Page](https://docs.microsoft.com/en-us/rest/api/mysql/singleserver/servers(2017-12-01)/create?tabs=HTTP)  and click on the "Try It" tab highlighted in green and login in with your Azure account.
